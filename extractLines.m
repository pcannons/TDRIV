function rects = extractLines(subpic, loc)
    ...
    horizProjMinima = find(horizproj >= 1.4*std(horizproj));
    ...
    % EXCHANGE *
    lineBreaks = mat2cell(horizProjMinima',1,diff([0,find(diff(horizProjMinima') >= 6),length(horizProjMinima)]));
...
%     end
    ...
    for i = 1:zSize(1)
        if (lineBreaks(i,2) - lineBreaks(i,1) < 7) 
            continue;
        end
